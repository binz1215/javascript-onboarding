...
function getDigitSum(page) { // 덧셈값 함수
  let sum = 0; //sum을 0으로 초기화한다.
  while (page > 0) {
    //page가 0이 될 때까지
    sum += page % 10; //page의 1의 자릿수를 더한 다음,
    page = parseInt(page / 10); //page의 1의 자릿수는 버린다. #parsInt()메서드는 정수를 반환.
  }
  return sum;
}

function getDigitMultiply(page) { // 곱셈값 함수
...
function getBigger(page) { // 더 큰거 비교
